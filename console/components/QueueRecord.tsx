...
import { ago, firstSentence, money, longDate, remedyPhrase } from "@/lib/format";
...
export function QueueRecord({ item }: { item: Case }) {
  ...
  return (
    <Link href={`/case/${item.case_id}`} className="record group">
      <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1">
        <StatusMark status={item.status} />
        <span className="micro">opened {ago(item.created_at)}</span>
        <span className="micro">notice dated {longDate(item.recall.recall_date)}</span>
      </div>

      <h2 className="record-title mt-4 max-w-[46ch]">{item.purchase.description}</h2>
      <p className="micro mt-1.5">{bought}</p>

      <p className="hazard mt-5 max-w-[62ch]">
        {firstSentence(item.recall.hazards[0] ?? item.recall.title)}
      </p>

      <p className="data mt-5 max-w-[70ch]" style={{ color: "var(--ink-2)" }}>
        {decisionLine(item)}
      </p>

      <p
        className="record-action mt-6 inline-flex items-center gap-2"
