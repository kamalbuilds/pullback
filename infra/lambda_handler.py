...
def _load_households() -> dict[str, list[dict]]:
    """The agent brain contract: agent.pullback_agent.households() ->
    {household_name: [purchase_dict, ...]}. Imported defensively at call
    time because that module is being written in parallel and may not
    exist yet, or may not yet define `households`. Either way this must
    not fabricate a household list; it returns empty and the caller logs
    why."""
    try:
    ...
    try:
        result = _households()
    except Exception as exc:  # the brain existing but throwing is still not our job to hide
        _log("pullback_agent_error", reason=str(exc), traceback=traceback.format_exc())
        return {}
    if not isinstance(result, dict):
        _log("pullback_agent_bad_shape", got=type(result).__name__)
        return {}
    return result
